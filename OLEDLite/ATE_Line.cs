...
namespace OLEDLite
{
    public class ATE_Line : TaskRun
    {
        ...
        public override void ATETask()
        {
                ...
                bin_idx++)
            {
                ...
                for(int iout_idx = 0; iout_idx < iout_cnt; iout_idx++)
                {
                    ...
                    row++;
                    _sheet.Cells[row, XLS_Table.A] = string.Format("Iout={0:0.00}mA", test_parameter.ioutList[iout_idx] * 1000);
                    ...
                    for (int vin_idx = 0; vin_idx < vin_cnt; vin_idx++)
                    {
                        ...
                        _sheet.Cells[row, XLS_Table.C] = test_parameter.ESwire_state ? string.Format("{0:0.0000}", measure_data[1]) : "0";
                        _sheet.Cells[row, XLS_Table.D] = test_parameter.ESwire_state ? string.Format("{0:0.0000}", measure_data[2]) : "0";
                        _sheet.Cells[row, XLS_Table.E] = test_parameter.ASwire_state ? string.Format("{0:0.0000}", measure_data[3]) : "0";
                        _sheet.Cells[row, XLS_Table.F] = test_parameter.ENVO4_state ? string.Format("{0:0.0000}", measure_data[4]) : "0";
        ...
        private void AddAllCurve(List<int> start_pos, List<int> stop_pos, int bin_idx, string X_axis)
        {
            ...
            info.Ytitle = "AVDD(V)";
            info.Y = "E";
            range_pos = _sheet.Range["D36", "J49"];
            AddCurve(
            _sheet, range_pos, info, start_pos, stop_pos
            );
            ...
            info.Ytitle = "DVDD(V)";
            info.Y = "F";
            range_pos = _sheet.Range["D52", "J65"];
            AddCurve(
            _sheet, range_pos, info, start_pos, stop_pos
            );
        ...
        private void LIRData(List<int> start_pos, List<int> stop_pos)
        {
            ...
            row++;
            row = 12;
            for (int line = 0; line < test_parameter.ioutList.Count; line++)
            {
                // ELVDD
